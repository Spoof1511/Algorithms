    ...
    const std::string& plate) 
{
   ...
   while(i<NUMBER_OF_AISLES){
    ...
    std::logic_error("Parking is full!");
}
...
void handle_departure(Cars& cars, Parking& parking_lot, const std::string& plate)
{
    Car& quitingCar=find_car(cars,plate);
    ParkAisle& aisle=parking_lot[quitingCar.getAisle()];
    ParkAisle aisleTemp;
    while (aisle.top()!=plate)
    {
        std::string& str=aisle.top();
        Car& movingMachine=find_car(cars,aisle.top());
        movingMachine.setTimesMoved(movingMachine.getTimesMoved()+1);
        aisleTemp.push(str);
        aisle.pop();
    }
    aisle.pop();
    std::cout<<quitingCar.getPlate()<<" moved "<<quitingCar.getTimesMoved()<<" times "<<std::endl;
    while (!aisleTemp.empty())
    {
        std::string& str=aisleTemp.top();
        aisle.push(str);
        aisleTemp.pop();
    }
}
...
{
    Cars::iterator iter;
    iter=std::find(cars.begin(),cars.end(),plate);
    if (iter==cars.end()) throw std::logic_error("Car doesnt exist!");
    return  *iter;
}
